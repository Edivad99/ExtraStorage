...
public class AdvancedCrafterTileDataParameterClientListener implements
    BlockEntitySynchronizationClientListener<Boolean> {

  @Override
  public void onChanged(boolean initial, Boolean hasRoot) {
    if (!hasRoot) {
      BaseScreen.executeLater(AdvancedCrafterScreen.class,
          screen -> screen.addSideButton(new AdvancedCrafterModeSideButton(screen)));
    }
  }
}
