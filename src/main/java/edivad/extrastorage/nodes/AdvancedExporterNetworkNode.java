    ...
    if (canUpdate() && ticks % upgrades.getSpeed() == 0 && level.isLoaded(pos)) {
      if (type == IType.ITEMS) {
        ...
        if (handler != null) {
          ...
          if ((filterSlot == SLOTS - 1 && itemFilters.getStackInSlot(filterSlot).isEmpty()) || (
              filterSlot >= SLOTS)) {
            filterSlot = 0;
          }
          ...
          if (!slot.isEmpty()) {
            ...
            if (upgrades.hasUpgrade(UpgradeItem.Type.REGULATOR)) {
              ...
              for (int i = 0; i < SLOTS; ++i) {
                if (API.instance().getComparer()
                    .isEqualNoQuantity(slot, itemFilters.getStackInSlot(i))) {
            ...
            if (stackSize > 0) {
              ItemStack took = network.extractItem(slot,
                  Math.min(slot.getMaxStackSize(), stackSize), compare, Action.SIMULATE);
        ...
        // and it's empty, go back to slot 0.
        // We also handle if we exceeded the maximum slot in general.
        if ((filterSlot == fluids.length - 1 && fluids[filterSlot] == null) || (filterSlot
            >= fluids.length)) {
          filterSlot = 0;
        }
          ...
          if (stack != null) {
            ...
            if (stackInStorage != null) {
              ...
              if (upgrades.hasUpgrade(UpgradeItem.Type.REGULATOR)) {
                ...
                for (int i = 0; i < SLOTS; ++i) {
                  if (API.instance().getComparer()
                      .isEqual(stack, fluidFilters.getFluid(i), IComparer.COMPARE_NBT)) {
